...
void Cache::lw(int addr, int *memory) {
    ...
    index = (addr >> 2) & 0xF;
    tag = addr >> 6;
    // std::cout << "L1: (offset: " << block_offset <<
    //     ", index: " << index << 
    //     ", tag: " << tag << ")" << std::endl;
    ...
    if (this->l1[index].index == index && this->l1[index].tag == tag && this->l1[index].valid) {
    ...
    if (!l1_hit) {
        tag = addr;
        // std::cout << "VIC: (offset: " << block_offset <<
        //     ", index: " << index << 
        //     ", tag: " << tag << ")" << std::endl;
        // std::cout << "vic: ";
        for (auto &entry : this->victim) {
            std::cout << (entry.valid ? entry.data : -1) << " ";
    ...
    if (!l1_hit && !vic_hit) {
        tag = addr >> 6;
        // std::cout << "L2: (offset: " << block_offset <<
        //     ", index: " << index << 
        //     ", tag: " << tag << ")" << std::endl;
