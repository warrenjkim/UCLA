...
void write_block_bitmap(int fd) {
  ...
  u8 block_bmp[BLOCK_SIZE];

  for (int block = 0; block < BLOCK_SIZE; block++) {
    int block_index = 8 * block;

    for (int i = 0; i < 8; i++) { // 8 block_index in a block
      int curr_index = block_index + i;

      if (curr_index < LAST_BLOCK || NUM_BLOCKS - 1 <= curr_index)
	block_bmp[block] = block_bmp[block] | (1<<i);
      else
	block_bmp[block] = block_bmp[block] & ~(1<<i);
    }
  }

  ssize_t size = sizeof(block_bmp);
  if (write(fd, &block_bmp, size) != size)
    errno_exit("write");
}
...
void write_inode_bitmap(int fd) {
  ...
  for (int block = 0; block < BLOCK_SIZE; block++) {
    ...
    for (int i = 0; i < 8; i++) { // 8 block_index in a block
      ...
      if (curr_index < LAST_INO || NUM_INODES <= curr_index)
	inode_bmp[block] = inode_bmp[block] | (1<<i);
      else
	inode_bmp[block] = inode_bmp[block] & ~(1<<i);
  ...
  ssize_t size = sizeof(inode_bmp);
  if (write(fd, &inode_bmp, size) != size)
    errno_exit("write");
}
...
void write_inode_table(int fd) {
  ...
  char* link_location = "hello-world";
  memcpy(hello_inode.i_block, link_location, strlen(link_location)+1);
...
void write_root_dir_block(int fd) {
  ...
  ssize_t bytes_remaining = BLOCK_SIZE; // bytes left

  //current entry
  struct ext2_dir_entry curr_entry = {0};
  dir_entry_set(curr_entry, EXT2_ROOT_INO, ".");
  dir_entry_write(curr_entry, fd);

  bytes_remaining -= curr_entry.rec_len;
...
void write_hello_world_file_block(int fd) {
  ...
  // write to file
  ssize_t str_size = sizeof(hello_world_str);
