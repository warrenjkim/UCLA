...
struct hash_table_v1 *hash_table_v1_create()
{
  ...
  int initno = pthread_mutex_init(&hash_table->mutex, NULL); // initialize
...
{
  int unlockno = 0;
  int lockno = pthread_mutex_lock(&hash_table->mutex); // lock
  
  if (lockno) // lock failed
    exit(lockno);
...
void hash_table_v1_destroy(struct hash_table_v1 *hash_table)
{
  ...
  int destroyno = pthread_mutex_destroy(&hash_table->mutex);
  if (destroyno)
    exit(destroyno);
  free(hash_table);
}
