...
struct local_file *create_local_file(const char *type,
                                     const unsigned char *content,
                                     const long int size);
...
void parse_args(int argc, char *argv[], struct server_app *app) {
  ...
  while ((opt = getopt(argc, argv, "b:r:p:")) != -1) {
    ...
    default: /* Unrecognized parameter or "-?" */
      fprintf(
          stderr,
          "Usage: server [-b local_port] [-r remote_host] [-p remote_port]\n");
      exit(-1);
      break;
    }
  }
...
void proxy_remote_file(struct server_app *app, int client_socket,
  ...
  address.sin_port = htons(app->remote_port);
...
struct local_file *create_local_file(const char *type,
  ...
  struct local_file *file =
      (struct local_file *)malloc(sizeof(struct local_file));
 ...
 *
 * This function returns the file type for the Content-Type field of the HTTP
 * header.
 *
 * @param file_name: The name of the file.
 *
