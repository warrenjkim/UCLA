...
def sgd_nesterov_momentum(w, dw, config=None):
    ...
    next_w = w + v_new + (momentum * (v_new - v))
    v = v_new
...
def rmsprop(w, dw, config=None):
    ...
    config['a'] = (decay_rate * a) + ((1 - decay_rate) * dw**2)
...
def adam(w, dw, config=None):
    ...
    config['t'] += 1
    config['v'] = (beta1 * v) + ((1 - beta1) * dw)
    config['a'] = (beta2 * a) + ((1 - beta2) * dw**2)
    v_tilde = (1 / (1 - (beta1**config['t']))) * config['v']
    a_tilde = (1 / (1 - (beta2**config['t']))) * config['a']
    next_w = w - ((learning_rate / (np.sqrt(a_tilde) - epsilon)) * v_tilde)
