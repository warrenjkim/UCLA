        ...
        for j in range(F):
            for r in range(out_height):
                for c in range(out_width):
                    out[i, j, r, c] = \
                    np.sum(w[j, ...] * \
                        padded[i, :, r * stride: r * stride + HH, c * stride: c * stride + WW]) + b[j]
        ...
        for j in range(F):
            for r in range(out_height):
                for c in range(out_width):
                    dw[j, ...] += \
                    dout[i, j , r, c] * \
                        xpad[i, :, r * stride: r * stride + f_height, c * stride: c * stride + f_width]

    for i in range(N):
        for j in range(F):
            for r in range(out_height):
                for c in range(out_width):
                    dx[i, :, r * stride: r * stride + f_height, c * stride: c * stride + f_width] += \
                    dout[i, j, r, c] * w[j, ...]
        ...
        for j in range(C):
            for r in range(dout_height):
                for c in range(dout_width):
                    max_i = x[i, j, r * stride: r * stride + pool_height, c * stride: c * stride + pool_width]
                    mask = (max_i == np.max(max_i))
                    dx[i, j, r * stride: r * stride + pool_height, c * stride: c * stride + pool_width] += dout[i, j, r, c] * mask
...
def spatial_batchnorm_forward(x, gamma, beta, bn_param):
    ...
    flat, cache = batchnorm_forward(x_reshape, gamma, beta, bn_param)
...
def spatial_batchnorm_backward(dout, cache):
    ...
    dout_reshape = np.reshape(dout_transpose, (N * H * W, C))
    flat, dgamma, dbeta = batchnorm_backward(dout_reshape, cache)
