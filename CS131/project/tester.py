...
def generate_test_suite_v2():
    ...
    tests_dir = Path("v2/tests")
    fails_dir = Path("v2/fails")
    test_names = [file.stem for file in tests_dir.iterdir()
                  if file.suffix == ".brewin"]
    fail_names = [file.stem for file in fails_dir.iterdir()
                  if file.suffix == ".brewin"]
...
#    return __generate_test_suite(2, [], ['test_type_mismatch']) #'test_overload1', 'test_return_default2' FAILS: 'test_invalid_param_passing3'
